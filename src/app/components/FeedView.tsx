...
export function FeedView({ currentUser, onPlayTrack, refreshFeed }: FeedViewProps) {
  ...
  const openInMusicApp = (shake: Shake) => {
    ...
    if (url) {
      ...
      // Fallback: search on the preferred platform
      ...
      <div className="p-4 space-y-3">
        {shakes.map((shake, index) => (
          <motion.article
            ...
            className="bg-purple-950/25 rounded-xl border border-purple-800/20 hover:border-purple-700/40 transition-all overflow-hidden"
          >
            {/* Reshake indicator */}
            {shake.reshakeFrom && (
            ...
            <div className="px-4 py-2 flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-gray-500 text-xs">@{shake.user.username}</span>
                  <span className="text-gray-600 text-xs">·</span>
                  <span className="text-gray-500 text-xs">{formatTimestamp(shake.timestamp)}</span>
                </div>
              </div>
            ...
            {!shake.track.spotifyEmbedUrl && (
              <div className="px-4 pb-3">
                <div className="bg-purple-900/20 rounded-xl p-3 flex gap-3 group cursor-pointer hover:bg-purple-900/30 transition-all border border-purple-800/10">
            ...
            <div className="px-4 pb-2.5 flex items-center gap-6">
              <button onClick={() => toggleLike(shake.id)} className="flex items-center gap-1.5 group">
                <Heart className={`w-5 h-5 transition-all ${shake.isLiked ? 'text-pink-500 fill-pink-500' : 'text-gray-400 group-hover:text-pink-500'}`} />
                <span className={`text-xs font-medium ${shake.isLiked ? 'text-pink-500' : 'text-gray-400'}`}>{shake.likes}</span>
              </button>

              <button onClick={() => setCommentsPostId(shake.id)} className="flex items-center gap-1.5 group">
                <MessageCircle className="w-5 h-5 text-gray-400 group-hover:text-blue-400 transition-colors" />
              ...
              <button onClick={() => setReshakeDialogShake(shake)} className="flex items-center gap-1.5 group">
                <Repeat2 className={`w-5 h-5 transition-all ${shake.isReshaked ? 'text-green-500' : 'text-gray-400 group-hover:text-green-500'}`} />
                <span className={`text-xs font-medium ${shake.isReshaked ? 'text-green-500' : 'text-gray-400'}`}>{shake.reshakes}</span>
              </button>

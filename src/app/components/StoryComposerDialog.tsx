            ...
            <div className="p-4 space-y-3 max-h-[75vh] overflow-y-auto">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-9 py-2 bg-purple-950/40 border border-purple-700/30 rounded-lg text-sm text-white placeholder-purple-400/50 focus:outline-none focus:border-purple-500"
                />
                {trackQuery.length > 0 && (
                  <button onClick={() => { setTrackQuery(''); setTrackResults([]); }} className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 hover:bg-purple-800/40 rounded-full">
                    <X className="w-3.5 h-3.5 text-purple-400/60" />
                  </button>
                )}
              </div>

              {(searching || trackResults.length > 0) && (
                <div className="rounded-xl border border-purple-700/30 bg-purple-950/50 overflow-hidden">
                  {searching && (
                    <div className="flex items-center justify-center py-3">
                      <Loader2 className="w-4 h-4 text-purple-400 animate-spin" />
                    </div>
                  )}
                  {!searching && trackResults.slice(0, 6).map((t: any) => (
                    <button key={t.id} onClick={() => { setSelectedTrack(t); setTrackQuery(''); setTrackResults([]); }}
                      className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-purple-800/30 transition-colors text-left border-b border-purple-800/20 last:border-0">
                      <img src={t.cover} alt="" className="w-9 h-9 rounded-lg flex-shrink-0 object-cover" />

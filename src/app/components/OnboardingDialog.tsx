...
export function OnboardingDialog({ onComplete }: OnboardingDialogProps) {
  ...
  return (
    <div className="fixed inset-0 bg-[#1E1440] z-50 flex flex-col items-center justify-center p-6">
      {/* Background glow */}
          ...
          {!isServiceStep ? (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                {([
                  { id: 'spotify', name: 'Spotify', sub: 'Le plus populaire', color: 'bg-fuchsia-500', icon: (
                  ...
                  { id: 'apple', name: 'Apple Music', sub: 'Pour les fans Apple', color: 'bg-gradient-to-br from-pink-500 to-purple-600', icon: <Music className="w-6 h-6 text-white" /> },
                  { id: 'deezer', name: 'Deezer', sub: 'Flow & découvertes', color: 'bg-gradient-to-br from-purple-600 to-violet-800', icon: (
                    <svg className="w-6 h-6 text-white" viewBox="0 0 24 24" fill="currentColor">
                      <rect x="0" y="18" width="4" height="4" rx="0.5"/>
                      <rect x="6.5" y="18" width="4" height="4" rx="0.5"/>
                      <rect x="6.5" y="13" width="4" height="4" rx="0.5"/>
                      <rect x="13" y="18" width="4" height="4" rx="0.5"/>
                      <rect x="13" y="13" width="4" height="4" rx="0.5"/>
                      <rect x="13" y="8" width="4" height="4" rx="0.5"/>
                      <rect x="19.5" y="18" width="4" height="4" rx="0.5"/>
                      <rect x="19.5" y="13" width="4" height="4" rx="0.5"/>
                      <rect x="19.5" y="8" width="4" height="4" rx="0.5"/>
                      <rect x="19.5" y="3" width="4" height="4" rx="0.5"/>
                    </svg>
                  )},
                  { id: 'youtube_music', name: 'YouTube Music', sub: 'Clips & sons', color: 'bg-red-600', icon: (
                    <svg className="w-6 h-6 text-white" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M12 0C5.376 0 0 5.376 0 12s5.376 12 12 12 12-5.376 12-12S18.624 0 12 0zm4.95 16.95H7.05V7.05h9.9v9.9zM9.525 9.15v5.7l4.95-2.85-4.95-2.85z"/>
                    </svg>
                  )},
                ] as const).map((svc) => (
                  <button
                    ...
                    className={`p-3 rounded-2xl border-2 transition-all ${
                      selectedService === svc.id
                        ? 'border-fuchsia-500/60 bg-fuchsia-500/5 scale-[1.02]'
